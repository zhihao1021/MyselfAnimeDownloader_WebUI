...
function _modify(ele, event) {
    ...
    switch (ele.textContent) {
        case "pause":
            $.post("/api/queue-modify", {"id": id, "modify": "pause"})
            ele.textContent = "play_arrow";
            break;
        case "play_arrow":
            $.post("/api/queue-modify", {"id": id, "modify": "resume"})
            ele.textContent = "pause";
            break;
        case "stop":
            $.post("/api/queue-modify", {"id": id, "modify": "stop"})
            break;
        case "keyboard_arrow_up":
            if (event.shiftKey) {
                $.post("/api/queue-modify", {"id": id, "modify": "highest"});
            }
            else {
                $.post("/api/queue-modify", {"id": id, "modify": "upper"});
            }
            break;
        case "keyboard_arrow_down":
            if (event.shiftKey) {
                $.post("/api/queue-modify", {"id": id, "modify": "lowest"});
            }
            else {
                $.post("/api/queue-modify", {"id": id, "modify": "lower"})
            }
            break;
    }
